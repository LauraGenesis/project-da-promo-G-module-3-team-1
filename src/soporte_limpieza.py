...
def col_capitalizar(dataframe):
    '''
    Capitaliza los nombres de las columnas del DataFrame.
    Esta función toma un DataFrame y cambia todos los nombres de las columnas y las capitaliza.
...
def limpiar_valores(dataframe):
    ...
    for col in dataframe.columns:
        # Verificar si el tipo de datos de la columna es una cadena
        if dataframe[col].dtype == 'object':
            # Reemplazar el símbolo de dólar ($) con una cadena vacía
            dataframe[col] = dataframe[col].str.replace("$", "")
            # Reemplazar la coma (,) con un punto (.)
            dataframe[col] = dataframe[col].str.replace(",", ".")
            # Reemplazar el guion bajo (_) con un espacio en blanco
...
#función para rellenar el número de empleado:
def rellenar_id(fila, conjunto):
    """
    Rellena los valores de la columna 'Employee_id' en una fila del DataFrame si el valor es 0.
    
    La función verifica si el valor de 'Employee_id' en la fila actual es igual a 0. Si es así, genera un número 
    aleatorio entre 1 y 20000 y verifica si ese número ya está presente en el conjunto dado como argumento. 
    Si el número generado ya está en el conjunto, genera otro número hasta encontrar uno que no esté en el conjunto. 
    Una vez que se encuentra un número único, se asigna a la columna 'Employee_id' de la fila actual. 
    El número único también se agrega al conjunto para evitar duplicados en futuras iteraciones.

    Args:
        fila (pandas.Series): Fila del DataFrame.
        conjunto (set): Conjunto que contiene los valores únicos de 'Employee_id' en el DataFrame.

    Returns:
        pandas.Series: La fila actual con el valor de 'Employee_id' actualizado, si es necesario.
    """
    if fila['Employee_id'] == 0:
        id = random.randint(1, 20000)
        while id in conjunto:
            id = random.randint(1, 20000)
        fila['Employee_id'] = id
        conjunto.add(id)
    return fila
